...
int cardEffectAdventturer(struct gameState *state) {
    ...
    int currentPlayer = NULL;//Bug here int currentPlayer = whoseTurn(state);
    int z = 0;
...
int cardEffectSmithy(int handPos, struct gameState *state) {
    int currentPlayer = whoseTurn(state);

    for (int i = 1; i < 3; i++) {//Bug here i = 0
        drawCard(currentPlayer, state);
    }
...
int cardEffectCouncilRoom(int handPos, struct gameState *state) {
    int currentPlayer = whoseTurn(state);
...
int cardEffectFeast(int choice1, struct gameState *state) {
    int temphand[MAX_HAND];
    int currentPlayer = whoseTurn(state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus) {
    ...
    switch (card) {
        ...
        case feast:
            return cardEffectFeast(choice1, state);
        ...
        case smithy:
            return cardEffectSmithy(handPos, state);
